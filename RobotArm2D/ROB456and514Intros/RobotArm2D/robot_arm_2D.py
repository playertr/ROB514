...
class DrawRobot(QWidget):
    ...
    def get_matrices(self):
        ...
        mat_ret['upperarm'] = self.translation_matrix(0, 0.5).dot(self.rotation_matrix(ang_shoulder))
        mat_ret['forearm']  = self.translation_matrix(len_upper_arm, 0).dot(self.rotation_matrix(ang_elbow))
        mat_ret['wrist']    = self.translation_matrix(len_forearm, 0).dot(self.rotation_matrix(ang_wrist))
        mat_ret['finger1']  = self.translation_matrix(len_wrist, 0).dot(self.rotation_matrix(ang_finger/2))
        mat_ret['finger2']  = self.translation_matrix(len_wrist, 0).dot(self.rotation_matrix(-ang_finger/2))
    ...
    def draw_arm(self, qp):
        ...
        upperarm_transform = self.transform_rect(rects['upperarm'], matrices['upperarm'])
        self.draw_rect(upperarm_transform, qp)

        f_m = matrices['upperarm'].dot(matrices['forearm'])
        forearm_transform = self.transform_rect(rects['forearm'], f_m)
        self.draw_rect(forearm_transform, qp)

        w_m = f_m.dot(matrices['wrist'])
        wrist_transform = self.transform_rect(rects['wrist'], w_m)
        self.draw_rect(wrist_transform, qp)

        f1_m = w_m.dot(matrices['finger1'])
        f1_transform = self.transform_rect(rects['finger1'], f1_m)
        self.draw_rect(f1_transform, qp)

        f2_m = w_m.dot(matrices['finger2'])
        f2_transform = self.transform_rect(rects['finger2'], f2_m)
        self.draw_rect(f2_transform, qp)
        ...
        # define a matrix to transform from arm frame to GUI frame
        # rect_transform = self.transform_rect(rects['base'], mat)
    ...
    def arm_end_pt(self):
        ...
        mat_accum = mat_accum.dot(matrices['upperarm'])
        mat_accum = mat_accum.dot(matrices['forearm'])
        mat_accum = mat_accum.dot(matrices['wrist'])
        mat_accum = mat_accum.dot(matrices['finger1'])
